...
Write-Host "🔧 AutoPilot Ventures - Simple Fix Script" -ForegroundColor Green
Write-Host ""
...
if ($gcloudCheck) {
    ...
    # Get project number using the correct format
    $projectNumber = gcloud projects describe $PROJECT_ID --format="value(projectNumber)"
    if (-not $projectNumber) {
        # Alternative way to get project number
        $projectNumber = gcloud projects list --filter="projectId=$PROJECT_ID" --format="value(projectNumber)"
    }
    ...
    if ($projectNumber) {
        ...
        Write-Host "❌ Could not get project number. Please check your gcloud configuration." -ForegroundColor Red
